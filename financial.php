...
if (isset($do) && $do != '' && isset($invoiceID) && $invoiceID != '') {
	if ($do == 'delete') {
		$sql_invoice_viewfordelete = mysqli_query($connection, "SELECT * FROM invoices WHERE invoice_code='$invoiceID'");
		$sql_invoice_vfd_result = mysqli_fetch_array($sql_invoice_viewfordelete);
		$sql_invoice_vfd_is_paid = $sql_invoice_vfd_result['is_paid'];
		if ($sql_invoice_vfd_is_paid == '0') {
		...
		elseif ($sql_invoice_vfd_is_paid == '1') {
			echo "به دلیل اینکه فاکتور پرداخت گردیده است، امکان حذف آن وجود ندارد.";
		}
	}
	elseif ($do == 'paid') {
	...
	$sql_invoices = mysqli_query($connection, "SELECT * FROM invoices ORDER BY invoice_code DESC");
	...
	$sql_invoices_of_user = mysqli_query($connection, "SELECT * FROM invoices ORDER BY invoice_code DESC LIMIT $start , $end");
			...
			<tr><?php
				...
				while($sql_invoices = mysqli_fetch_array($sql_invoices_of_user)){
					...
					if ($sql_invoice_is_paid == '0') {
						...
						$sql_is_paid_change_link = "<a href=\"financial.php?do=paid&invoiceID=$sql_invoice_code\">تغییر وضعیت به پرداخت شده</a>";
					}
