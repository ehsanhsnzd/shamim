...
?>
		...
		<table id="financial-invoices-table" width='80%'>
			<tr>
				<th>ردیف</th>
			...
			<tr><?php
				...
				while($sql_invoices = mysqli_fetch_array($sql_invoices_of_user)){
					...
					$sql_invoice_create_date = jdate('Y/m/d h:i a',strtotime( $sql_invoices['invoice_create_date'].' +210 minutes'),'none','Iran/Tehran','fa');
          $sql_invoice_deposit_date=$sql_invoices['deposit_date'];
					...
					$username= $sql_invoices['username'];

					if (is_null($sql_invoice_deposit_date)) {
						$sql_invoice_deposit_date = '-';
					}else{
                        $sql_invoice_deposit_date = jdate('Y/m/d h:i a',strtotime( $sql_invoices['deposit_date'].' +210 minutes'),'none','Iran/Tehran','fa');
                    }
					if (!empty($username)){
					$userinfo="&username=$username";}
					if ($sql_invoice_is_paid == '0') {
						$sql_invoice_is_paid= '<span class="red-text">پرداخت نشده</span>';
						$sql_payment_link ="<a href=\"financial_mellat.php?do=delete&invoiceID=$sql_invoice_code\"><span class=\"adminpanel-delete-icon\"></span></a>";
						$sql_is_paid_change_link = "<a href=\"financial_mellat.php?do=paid&invoiceID=$sql_invoice_code&pay=1&username=$username\">تغییر وضعیت به پرداخت شده</a>";
					}
					...
					$sql_payment_link ="<a href=\"financial_mellat.php?do=delete&invoiceID=$sql_invoice_code&username=$username\"><span class=\"adminpanel-delete-icon\"></span></a>";
						$sql_is_paid_change_link = "<a href=\"financial_mellat.php?do=paid&invoiceID=$sql_invoice_code&pay=0&username=$username\">تغییر وضعیت به پرداخت نشده</a>";
					}
