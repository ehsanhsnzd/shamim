...
if(!$rs->eof)
{
	...
	while(!$rs->eof)
	{
		...
		?>
			...
			?>
			<?=currency(1);?><?=float_opt($total1,2)?> <?=currency(2);?>
			</td>
			<td class="hidden-phone hidden-tablet">
			<?
			$total2=0;
			...
			?>
			<?=currency(1);?><?=float_opt((-1*$total2),2)?> <?=currency(2);?>
			</td>
			<td><span class="price"><b><?=currency(1);?><?=float_opt($total1+$total2,2)?> <?=currency(2);?></b></span></td>
			<td>
				<?
				$sql="select paypal,moneybookers,dwolla,qiwi,webmoney,bank_name,bank_account from users where id_parent=".$rs->row["user"];
				$dr->open($sql);
				if(!$dr->eof)
				{
	...
	<?
	echo(paging2($n,$str,$kolvo,$kolvo2,"index.php","&d=1"));
}
else
{
