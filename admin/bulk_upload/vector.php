...
You should upload files here <b><?=surl?><?=site_root?><?=vectorpreupload?></b> on FTP<br><br>
...
<table border=0 cellpadding=7 cellspacing=1 class=table_admin style="width:700px">
<tr>
<th><b><?=word_lang("file")?></b></th>
<th><b><?=word_lang("title")?>/<?=word_lang("description")?>/<?=word_lang("keywords")?><?if($site_model){echo("/".word_lang("model property release"));}?></b></th>
...
<td class=tab6 align="center"><input type="checkbox" name="prints" onclick="mdefault('prints')" <?if($site_prints==true){?>checked<?}?>></td>
</tr>
...
$bfiles=array();
$cfiles=array();
if(file_exists($_SERVER["DOCUMENT_ROOT"].site_root.vectorpreupload))
{
$dir = opendir ($_SERVER["DOCUMENT_ROOT"].site_root.vectorpreupload);
...
<td class=tab<?if($i%2==0){echo(4);}else{echo(6);}?> align="center"><input type="checkbox" name="prints<?=$i?>" <?if($site_prints==true){?>checked<?}?>></td>
</tr>
