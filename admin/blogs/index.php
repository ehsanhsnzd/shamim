...
if($search!="")
{
	...
	if($search_type=="text")
	{
		$com2.=" and (title like '%".result($search)."%' or content like '%".result($search)."%') ";
	}
...
$sql="select id_parent,title,content,data,user,published,photo,categories from blog where id_parent>0 ";
...
<th width="70%"><?=word_lang("title")?>/<?=word_lang("content")?></th>


<th class="hidden-phone hidden-tablet"><?=word_lang("categories")?></th>
...
<a href="../customers/content.php?id=<?=user_url($rs->row["user"])?>"><?=$rs->row["user"]?></a></div>
</td>
<td><b><?=$rs->row["title"]?></b><br>
<?
$content=str_replace("\n","<br>",$rs->row["content"]);
...
<td class="hidden-phone hidden-tablet"><?=$rs->row["categories"]?></td>
