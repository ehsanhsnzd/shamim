...
mysqli_query($connection, "UPDATE orders2 set read_m=1 WHERE order_id = $id");
...
$sql_order_details = mysqli_query($connection, "SELECT * FROM orders2 WHERE order_id = '$id'");
        ...
        if(isset($_POST['submit'])){
			...
			if (!isset($edited_order_pp) || $edited_order_pp == '' || !isset($edited_order_dp) || $edited_order_dp == '' || !isset($edited_order_status) || $edited_order_status == ''){
				...
				$sql = "UPDATE orders2 SET order_width = '$edited_order_width' , order_height = '$edited_order_height' , order_bijak_code = '$edited_order_bijak' , order_print_permission = '$edited_order_pp' , order_delivery_permission = '$edited_order_dp' , order_last_status = '$edited_order_status' WHERE order_id = '$id'";
				...
				if ($connection->query($sql) == TRUE  &&  !in_array($sql_od_user,$site_users)) {
                    ...
                    $resp = $gate->SendSMS('جناب ' . $name . $family . 'سفارش شما به وضعیت ' . $sql_od_ls . 'درآمد' . $text_type . ' '.$result_main_page_name, $smsnum, $Receptors);
...
<a href=\"".$sql_od_file4."\" >دانلود</a>";
					...
					echo "<table class=\"order-details-status-div\">";
						...
						echo "<tr>";
							...
							echo "<td>";
								...
								echo	"<option value=\"1\" $od_ls_1>در انتظار بررسی</option>
        ...
        echo "<p>دلیل تعلیق: <input type=\"text\" name=\"order-sms\" class=\"order-edit-input\" value=\"\"></p>";
