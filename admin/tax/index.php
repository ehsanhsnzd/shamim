...
<a class="btn btn-success toright" href="content.php"><i class="icon-book icon-white"></i> <?=word_lang("add")?></a>
...
if(!$rs->eof)
{
	...
	<div class="table_t2"><div class="table_b"><div class="table_l"><div class="table_r"><div class="table_bl"><div class="table_br"><div class="table_tl"><div class="table_tr"><table border="0" cellpadding="0" cellspacing="0" class="table_admin" style="width:100%">
	<tr>
	<th><b><?=word_lang("enabled")?>:</b></th>
	<th><b><?=word_lang("title")?>:</b></th>
	<th><b><?=word_lang("rates depend on")?>:</b></th>
	...
	{	
		...
		<td>
			<select name="rates_depend<?=$rs->row["id"]?>" style="width:150px">
				<option value="1" <?if($rs->row["rates_depend"]==1){echo("selected");}?>><?=word_lang("shipping address")?></option>
				<option value="2" <?if($rs->row["rates_depend"]==2){echo("selected");}?>><?=word_lang("billing address")?></option>
			</select>
		</td>
		<td align="center"><input name="price_include<?=$rs->row["id"]?>" type="checkbox" <?if($rs->row["price_include"]==1){echo("checked");}?>></td>
		<td><input name="rate_all<?=$rs->row["id"]?>" type="text" style="width:50px" value="<?=$rs->row["rate_all"]?>">&nbsp;<select name="rate_all_type<?=$rs->row["id"]?>" style="width:60px">
