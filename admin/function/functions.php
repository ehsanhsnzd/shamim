            ...
            if ($predel >= $r_page) {
                ...
                for ($p = 1; $p < $predel2 + 1; $p++) {
                    if (($p - 1) * $r_kolvo2 < $r_page and $p * $r_kolvo2 >= $r_page) {
                        ...
                        if ($r_page > 1) {
                            $vivod .= "<a href='" . $r_file . "?str=" . ($r_page - 1) . $r_perem . "' class='paging'>&#171; " . word_lang("previous") . "</a> ";
                        }

                        for ($s = ($p - 1) * $r_kolvo2 + 1; $s < $p * $r_kolvo2 + 1; $s++) {
                            if ($s <= $predel) {
                                if ($r_page == $s) {
                                    $vivod .= "<span class='paging2'>" . $r_page . "</span> ";
                                } else {
                                    $vivod .= "<a href='" . $r_file . "?str=" . $s . $r_perem . "' class='paging'>" . $s . "</a> ";
                                }
                            }
                        }
                        if ($r_page + 1 <= $predel) {
                            $vivod .= "<a href='" . $r_file . "?str=" . ($r_page + 1) . $r_perem . "' class='paging'>" . word_lang("next") . " &#187;</a> ";
                        }
    ...
    while (!$dp->eof) {
        if ($nlimit < 5000) {
            ...
            for ($i = 0; $i < $otstup; $i++) {
                $otp .= "&nbsp;&nbsp;";
                if ($i == 2) {
                    $color = "style='background-color:#CC9'";
                }
            }


            if ($dp->row["id"] != $iid) {
                $itg .= "<option $color value='" . $dp->row["id"] . "' " . $sel . ">" . $otp . $dp->row["title"] . "</option>";
                $color = "";
            }
...
function buildmenufirst2($t_id, $t_select, $otstup, $iid)
{
    global $db;
    global $dr;
    $dp = new TMySQLQuery;
        ...
        while (!$dp->eof) {
			...
			*/
            ...
            if ($nlimit < 5000) {
                ...
                if ($ttl == "فروشگاه") {
                    $ttl = "<img src='../images/shop-btn.png'  width='100px'/>";
                }
                ...
                $itg .= "<li class='dropdown mega-menu'>
				<a  href=\"" . category_url($dp->row["id"], $dp->row["url"]) . "\" class='dropdown-toggle' data-toggle='dropdown'>" . $ttl . "</a>\n
			...
			*/
            ...
            if ($nlimit < 5000) {
				...
				<div class='col-lg-3  col-sm-3 col-md-3 pull-right text-right'>
                          	<a href='" . $dp->row["url"] . "'> <h4 class='hidden-xs'>	" . $ttl . "</h4> </a>\n
				 ";

                buildmenulast6($dp->row["id"]);
        ...
        while (!$dp->eof) {
			...
			*/
            ...
            if ($nlimit < 5000) {
                ...
                $itg .= "<li  class=\"dropdown1 menu\"><a  href=\"" . category_url($dp->row["id"], $dp->row["url"]) . "\" title=\"" . $ttl . "\" >" . $ttl . "</a>\n";
                if (!$zp) {
                    $itg .= "</li>";
                }
                buildmenuchild6($dp->row["id"]);
                if ($zp) {
                    $itg .= "</li>";
                }
            }
        ...
        while (!$dp->eof) {
            ...
            */
            ...
            if ($nlimit < 5000) {
                ...
                if ($nn == 0) {
                    $itg .= "
				<ul class='dropdown-menu dropdown1-menu'>\n";
                }
...
function buildmenubot($t_id)
{
    ...
    if (!$dp->eof) {
        while (!$dp->eof) {
            ...
            */
            ...
            if ($nlimit < 5000) {
                ...
                //          $itg1 = ['text' => $ttl, 'callback_data' => "$menuid"];
                ...
                //  $itg2 = ['text' => $ttl, 'callback_data' => "$menuid"];
        ...
        //   $itg[]=[['text' => 'صفحه اول', 'callback_data' => "5"]];
    }
...
function isphoto($t_id)
{
    ...
    $sql = "select id_parent from vector where id_parent=$t_id";
    $dp->open($sql);
    if (!$dp->eof) {
        return true;
    } else {
...
function currency($param, $cr = true)
{
    ...
    if (!$site_credits or $cr == false) {
        if ($param == 1 and ($currency_code1 == "GBP" or $currency_code1 == "JPY" or $currency_code1 == "USD")) {
            return $currency_code2;
        }
        if ($param == 2 and $currency_code1 != "GBP" and $currency_code1 != "JPY" and $currency_code1 != "USD") {
...
function subscription_add($subscription_id)
{
    ...
    if (!$dp->eof) {
        ...
        $sql = "insert into subscription_list (title,data1,data2,user,approved,bandwidth,bandwidth_limit,subscription,subtotal,discount,taxes,total,billing_firstname,billing_lastname,billing_address,billing_city,billing_zip,billing_country,download_limit ,download,  date_limit_user ) 
  values ('" . $dp->row["title"] . "'," . mktime(date("H"), date("i"), date("s"), date("m"), date("d"), date("Y")) . "," . (mktime(date("H"), date("i"), date("s"), date("m"), date("d"), date("Y")) + 3600 * 24 * $dp->row["days"]) . ",'" . result($_SESSION["people_login"]) . "',0,0," . $dp->row["bandwidth"] . "," . (int)$subscription_id . "," . $subtotal . "," . $discount . "," . $taxes . "," . $total . ",'" . result(@$_SESSION["billing_firstname"]) . "','" . result(@$_SESSION["billing_lastname"]) . "','" . result(@$_SESSION["billing_address"]) . "','" . result(@$_SESSION["billing_city"]) . "','" . result(@$_SESSION["billing_zip"]) . "','" . result(@$_SESSION["billing_country"]) . "','" . result(10) . "','" . result(0) . "',now())";
        $db->execute($sql);
...
function credits_balance()
{
    ...
    if (isset($_SESSION["people_login"])) {
        ...
        for ($i = 0; $i < count($expiration_date) - 1; $i++) {
            $sql = "select quantity,expiration_date from credits_list where user='" . result($_SESSION["people_login"]) . "' and approved=1   order by data";
            $dp->open($sql);
            while (!$dp->eof) {
        ...
        while (!$dp->eof) {
            if ($dp->row["percentage"] > 0) {
                if ((int)$_POST["coupons"] == $dp->row["id_parent"]) {
                    $tip = 1;
                    $price = $dp->row["percentage"];
                    $coupons[count($coupons)] = $dp->row["id_parent"];
...
function bandwidth_download_add($login)
{
    global $db;
    $dp = new TMySQLQuery;
    $dp->connection = $db;

    $sql = "select   SUM(CASE WHEN (date_limit_user > CURRENT_TIMESTAMP() - INTERVAL 1 DAY ) THEN 1
           ELSE 0
           END) AS daily_limit
            ,id_parent,download_limit,download from subscription_list where user='" . $login . "' and bandwidth<bandwidth_limit and approved=1";
    $dp->open($sql);

    if (!$dp->eof && $dp->row["daily_limit"]  &&  $dp->row["download_limit"] > $dp->row["download"]) {
  $sql = "update subscription_list set download=download+1   where id_parent=" . $dp->row["id_parent"] ;
        $db->execute($sql);
    }
    elseif (!$dp->eof && $dp->row["daily_limit"]==0) {

        $sql = "update subscription_list set download=0 , date_limit_user = now() where id_parent=" . $dp->row["id_parent"];
        $db->execute($sql);
    }
    else {
        die("محدودیت روزانه");
...
function show_preview($id, $type, $type_preview, $type_url, $preview_server1 = "", $preview_folder = "", $seo_view = true)
{
    ...
    if ($type == "vector") {
        ...
        if (($preview_server1 == "" or $preview_folder == "") and !$flag_remote) {
            ...
            if (!$dp->eof) {
                ...
                if ((int)$dp->row["flash_width"] != 0) {
                    $flash_width = $dp->row["flash_width"];
                }
                if ((int)$dp->row["flash_height"] != 0) {
                    $flash_height = $dp->row["flash_height"];
                }
    ...
    if (!$flag_remote and $flag_seo_url and !preg_match('/icon_(photo|video|audio|vector)/i', $preview_url) and $seo_view) {
        ...
        $seo_mass = array();
        preg_match_all('|CwJ5FNgkpqFKle([0-9]*)\/[0-9]*\/thumb([0-9]*)\.([a-z0-9]*)$|Uis', $preview_url, $seo_mass);
...
function show_captcha()
{
    ...
    if (preg_match("/error/i", $captcha_text) or !$site_captcha) {
		...
		<input name='rn1' id='rn1' type='text' value='' class='ibox' style='width:100px'><input name='rn2' id='rn2' type='hidden' value='" . $rr . "'></td></tr></table><div id='error_rn1' name='error_rn1'></div>";
    } else {
