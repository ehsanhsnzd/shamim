...
$res="";
		...
		if($file_form)
		{
			...
			<td colspan='4'>".word_lang("use iptc info").": <input name='photo_iptc' type='checkbox' checked></td>
			</tr>";
		}
					...
					if(!$flag_storage)
					{
						...
						if(file_exists($_SERVER["DOCUMENT_ROOT"].$url))
						{
							...
							else
							{
								if(isset($_SESSION["entry_admin"]))
								{
									$rurl="<div class='content_exif'><b>".word_lang("file").":</b> <a href='".$url."'>".$dd->row["url"]."</a><br> </div>";
								}
							}
						}
					}
...
{if($admin_fields[$i]=="title")
	{
	$form_result.="<input type=\"submit\" value=\"نمایش زیرشاخه \" name=\"submitcat\"/>";
	}
...
//Upload zip preview
if(strtolower($nf[count($nf)-1])=="zip" || strtolower($nf[count($nf)-1])=="tif" || strtolower($nf[count($nf)-1])=="psd"  and !preg_match("/text/i",$_FILES["preview2"]['type']))
{
	if($_FILES["preview2"]['size']>0)
	{
...
while(!$ds->eof)
{
	if($ds->row["shipped"]!=1)
	{
		...
		if($_FILES["vector_sale".$ds->row["id_parent"]]['size']>0 )
		{
			if($flag==true)
			{
...
if(!preg_match("/\.jpg$|\.tif$/i",$_FILES["photo"]["name"]))
{
	$flag=false;
}
