...
<form action="<?= $_SERVER['PHP_SELF'] ?>" method="post"  id="service-add-form">
    شماره فاکتور :    <input name="search_num" type="text" id="search_num">
        ...
        <select name="catagory"   id="service-name" style="display:none" >
...
$pay=$_POST['pay'];
	$sql_orders_number = mysqli_query($connection, "SELECT * FROM orders2 where order_type LIKE '%ریسو%' ORDER BY order_id");
...
$order_of_user_query= "SELECT * FROM orders2 where  order_print_permission=0 and  order_type LIKE '%ریسو%' ORDER BY order_id DESC LIMIT $start , $end";
}  
...
elseif (isset($_POST['search_num']) &&  $_POST['catagory']==0 ){
 ...
 $pay_query="order_last_status<=0";}
		...
		$order_of_user_query =  "SELECT * FROM orders2 where  factor LIKE '%$search_num%'  and $pay_query and  order_type LIKE '%ریسو%' ORDER BY order_id DESC LIMIT $start , $end" ;
	}
...
elseif (isset($_POST['catagory']) && $_POST['catagory']>0 ){
	...
	$pay_query="order_last_status>0";}elseif($pay==0){
		...
		$order_of_user_query =  "SELECT * FROM orders2 where  cat =$catagory and factor LIKE '%$search_num%' and $pay_query and  order_type LIKE '%ریسو%' ORDER BY order_id DESC LIMIT $start , $end" ;
	}
	else{
	
	
	$order_of_user_query=  "SELECT * FROM orders2 where order_file1!='' and order_last_status>0 and  order_type LIKE '%ریسو%' ORDER BY order_id DESC LIMIT $start , $end" ;
}
 ...
 if ($_SESSION['print_admin_name']!='mousavi'){ 				
		...
		echo "<td>";
                        ...
                        <input name="input_id_last_status" type="text" style="display:none;" value="<?=$sql_order_id?>"/>
                         <input name="accept_last_status" type="text" style="display:none;" value="5"/> 
                    ...
                    <?php  echo "</td>";
                        ...
                        <input name="input_id_last_status_2" type="text" style="display:none;" value="<?=$sql_order_id?>"/>
                         <input name="accept_last_status_2" type="text" style="display:none;" value="5"/> 
