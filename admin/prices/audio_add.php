...
$sql="insert into audio_types (types,price,priority,title,license,shipped) values ('".result($_POST["types"])."',".(float)$_POST["price"].",".(int)$_POST["priority"].",'".result($_POST["title"])."',".(int)$_POST["license"].",".$shipped.")";
$db->execute($sql);

//define id
$sql="select id_parent from audio_types order by id_parent desc";
$dr->open($sql);
$id=$dr->row['id_parent'];
...
if((int)$_POST["addto"]==1)
{
	...
	$sql="select id_parent,server1 from audio order by id_parent";
	$rs->open($sql);
	while(!$rs->eof)
	{
		if($shipped!=1)
...
header("location:index.php?d=3&items_count=".$items_count."&type=add");
?>