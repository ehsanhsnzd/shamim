...
while(!$rs->eof)
{
	...
	while(!$dr->eof)
	{
		...
		$sql="update audio_types  set title='".result($_POST["title".$dr->row["id_parent"]])."',types='".result($_POST["types".$dr->row["id_parent"]])."',price=".(float)$_POST["price".$dr->row["id_parent"]].",priority=".(int)$_POST["priority".$dr->row["id_parent"]].",shipped=".$shipped." where id_parent=".$dr->row["id_parent"];
		$db->execute($sql);

		if(isset($_POST["delete".$dr->row["id_parent"]]))
		{
			$sql="delete from audio_types where id_parent=".$dr->row["id_parent"];
			$db->execute($sql);
		}
...
header("location:index.php?d=3&type=change");
?>