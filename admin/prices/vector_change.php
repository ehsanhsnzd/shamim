...
while(!$rs->eof)
{
	...
	while(!$dr->eof)
	{
		...
		$sql="update vector_types  set title='".result($_POST["title".$dr->row["id_parent"]])."',types='".result($_POST["types".$dr->row["id_parent"]])."',price=".(float)$_POST["price".$dr->row["id_parent"]].",priority=".(int)$_POST["priority".$dr->row["id_parent"]].",shipped=".$shipped." where id_parent=".$dr->row["id_parent"];
		$db->execute($sql);
