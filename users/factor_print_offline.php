...
function offset_detail($c,$order_type,$quantity,$unit_price,$total_price,$all_total)
{
    if ($total_price!=0) {
...
$invoice_info_query= mysqli_query($connection, "select * from factor where id=$invoiceID");
...
$sql_username_name = mysqli_query($connection, "SELECT display_name FROM user WHERE username='$operator'");
                    ...
                    while($sql_offset = mysqli_fetch_array($sql_orders_of_offset)){
                        ...
                        if($sql_order_last_status!=6 ){
                            if($sql_order_paperprice==0){
                                ...
                                $all_total=offset_detail($c,'خط تا افقی',$papernum,$linebreak_value,$sql_order_linebreak,$all_total);
								 $all_total=offset_detail($c,'خط تا عمودی',$papernum,$linebreaka_value,$sql_order_linebreaka,$all_total);
								 $all_total=offset_detail($c,'خط تا عمودی',$papernum,$linebreaka_value,$sql_order_linebreaka,$all_total);
								  $all_total=offset_detail($c,'سرچسب',$papernum,$sarchasb_value,$sql_order_sarchasb,$all_total);
