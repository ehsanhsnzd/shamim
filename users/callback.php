...
<?
	...
	///////////////// PAY REQUEST
		...
		if ($client->fault) {
			...
			//print_r($result);
			//echo '</pre>';
      header("Location: ../users/financial.php?err=nopayment");
      die();
		}
		else {
	 ...
	 if($invoice==0 && !empty($user_balance)){
		...
		mysqli_query($connection, "SET character_set_connection = 'utf8'");
		 ...
		 mysqli_query($connection , "insert into credits_list (title,data,user,quantity,approved,payment,credits,expiration_date,quantity_total) values ('پرداخت درگاه ملت - افزایش حساب','".date('Y-m-d H:i:s')."','".$user_balance."',".(float)$amount.",1,0,0,0,".(float)$quantity_total.")");
 ...
 mysqli_query($connection , "insert into credits_list (title,data,user,quantity,approved,payment,credits,expiration_date,quantity_total) values ('پرداخت درگاه ملت','".date('Y-m-d H:i:s')."','".$user_balance."',".(float)$amount.",1,0,0,0,".(float)$quantity_total.")");
	 ...
	 $amount_sub=0- $amount;
		...
		if ($connection->query($sql_paid_credit) === TRUE) {
				...
				$sql_paid_invoice = "UPDATE factor SET is_paid = '1',date_show= now(),order_last_status='6',order_description='اینترنتی - درگاه ملت' WHERE id = $invoice";
...
";
            ...
            mysqli_query($connection , "UPDATE orders2 SET  order_last_status = '2' , order_submit_date=now()  WHERE order_invoice_code = $invoice");
            mysqli_query($connection , "UPDATE orders1 SET  order_last_status = '2' , order_submit_date=now()  WHERE order_invoice_code = $invoice") ;
            mysqli_query($connection , "UPDATE orders3 SET  order_last_status = '2' , order_submit_date=now()  WHERE order_invoice_code = $invoice") ;
...
<br>
";
mysqli_query($connection, "update users set last_date=now() WHERE login = '$user_balance'");
          ...
          if(isset($_SESSION["REFERER_URL"]) and preg_match("/checkout/i",$_SESSION["REFERER_URL"]))
                {
                  ...
                  header("Location: ../users/financial.php");
                    die();
                }
