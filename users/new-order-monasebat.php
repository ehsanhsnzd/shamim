...
<h2 class="user-panel-sheet-h2">سفارش جدید</h2>
        ...
        <div style="background-color:#CCC; padding:10px 10px 10px 10px">
           <h4>   مناسبت ها </h4>
                <?
				...
				$month = $_GET['month'];
				  ...
				  $sel_day= mysqli_query($connection,"select * from date_month where month_id=$month");
						 ...
						 while($day_row=mysqli_fetch_array($sel_day)){ 
						   ...
						   $monthid=$day_row['month_id'];
						   $dayid=$day_row['id'];
						   
							echo'<a id="month-day"  href="?service='.$_GET['service'].'&type='.$_GET['type'].'&month='.$monthid.'&dayid='.$dayid.'"  >'. $day_row['text']. '</a> <br>';
        ...
        <br>
<center>
<a href="?service=<?=$_GET['service']?>&quantity=1th&type=3x4" <? if ($_GET['type']=='3x4'){echo "id='type-order-print-active'"; }else{ echo "id='type-order-print'";}?> ><br>
 داربست<br />

3x4</a> 
<a href="?service=<?=$_GET['service']?>&quantity=1th&type=2x3" <? if ($_GET['type']=='2x3'){echo "id='type-order-print-active'"; }else{ echo "id='type-order-print'";}?> ><br>
 داربست<br />
                              ...
                              <br/>
                               ...
                               <span>1. ابعاد:
                                ...
                                <label for="order-file-2">عرض سفارشی : </label> 
                                 <input type="text" name="order-custom-width" id="upload-div-order-width" placeholder="پیشفرض"     value="<?=$service_size_w?>"  >
                         </div>        <div style="display:inline-block;"><br>

                                <label for="order-file-2">طول سفارشی : </label> 
                                  <input type="text" name="order-custom-height" id="upload-div-order-height" placeholder="پیشفرض"  value="<?=$service_size_h?>">
                          </div>
                                </span><br/>
...
<center>    <ul class="enlarge">
				...
				<?
				 ...
				 if (!empty($dayid)){
					 $selgq= mysqli_query($connection,"select * from gallery where service=$serv  and monthid  = '$monthid' and dayid='$dayid'");}
					 else{
						  $selgq= mysqli_query($connection,"select * from gallery where service=$serv  and monthid  = '$monthid'    ");
						 }
                        ...
                        <div id="order-form-addition-div">
                          ...
                          (500 تومان)
                              تعداد حلقه :   <input name="halghe" value="4"  id="halghe" style="width:50px" disabled >
                                ...
                                <p>
                                      <input type="radio" name="order_make_format_beat" id="order_make_format_beat2" value="h" <? if($_GET['type']=='3x4' ||$_GET['type']=='2x3'){echo 'checked';}?>>                                      
                                    داربست گزاری عرض 
                                (1500 تومان)                                
                          <p><br/>
                           <script>
...
function uploadFile(){
 ...
 formdata.append("file3", file);
	...
	formdata.append("order-custom-height", height);
		formdata.append("rollmeter", height);
