...
{ $this->HandleError( "پسوند فایل ناشناس است");}
	$reserveq="insert into gallery set photo= 'Banner-"."',ext='$extension', service='$service' , type='$type',datef='$datef' ,datename='$datename' ";

mysqli_query($connection,"insert into gallery set photo= 'Banner-"."',ext='$extension', service='$service' , type='normal',datef='$datef' ,datename='$datename' ");	

mysqli_query($connection,"insert into gallery set photo= 'Banner-"."',ext='$extension', service='$service' , type='small',datef='$datef' ,datename='$datename' ");	
...
if(move_uploaded_file($_FILES['userfile']['tmp_name'],$upload_path . "Banner-".$service."-".$photo_id.$extension)) { 
 ...
 copy("../banner/Banner-".$service.'-'.$photo_id.$extension ,"../banner/Banner-".$service.'-'.($photo_id-1).$extension ) ;
 copy("../banner/Banner-".$service.'-'.$photo_id.$extension ,"../banner/Banner-".$service.'-'.($photo_id-2).$extension ) ;
...
<br>
       ...
       <? if($drop==36){
		   ?> تاریخ:
            <input type="text" id="datepicker1"  name="datef"/><br>
...
<br>
       ...
       <br />
            تاریخ:
           <input type="text" id="datepicker3"  name="date_from"/>
            <br>
            <br>
            ماه :
...
$month=$_POST['month'];
                        ...
                        $dbresult=mysqli_query($connection, "SELECT * FROM months ");
