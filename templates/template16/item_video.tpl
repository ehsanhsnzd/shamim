...
{if duration}<span><b>{WORD_DURATION}:</b> {DURATION}</span>{/if}
{if format}<span><b>{WORD_FORMAT}:</b> {FORMAT}</span>{/if}
{if ratio}<span><b>{WORD_RATIO}:</b> {RATIO}</span>{/if}
{if rendering}<span><b>{WORD_RENDERING}:</b> {RENDERING}</span>{/if}
{if frames}<span><b>{WORD_FRAMES}:</b> {FRAMES}</span>{/if}
{if holder}<span><b>{WORD_HOLDER}:</b> {HOLDER}</span>{/if}
{if model}
{MODEL}
{/if}
