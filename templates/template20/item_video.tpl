<div class="row">
    <div class="col-lg-6 col-md-6" style="min-width:500px">
			...
			{if format}<span><b>{lang.clip format}:</b> {FORMAT}</span>{/if}
			{if ratio}<span><b>{lang.aspect ratio}:</b> {RATIO}</span>{/if}
			{if rendering}<span><b>{lang.field rendering}:</b> {RENDERING}</span>{/if}
			{if frames}<span><b>{lang.frames per second}:</b> {FRAMES}</span>{/if}
        ...
        <ul class="social-links colored circle" style="margin-top:7px"> 
        	<li><a href="http://www.facebook.com/sharer/sharer.php?s=100&p[title]={SHARE_TITLE}&p[summary]={SHARE_TITLE}&p[url]={SHARE_URL}&&p[images][0]={SHARE_IMAGE}" target="_blank"> <i  class="fa fa-facebook"></i></a></li> 
