...
if(fl==false){ii[fid]=0;}
//alert(ii[fid]);
		var progress = new FileProgress(file,  this.customSettings.upload_target);
		progress.setProgress(percent);
		if(ii[fid]==0)
		{
		progress.setStatus("Uploading original photo...");
		}
		else
		{
		progress.setStatus("Generating thumbs...");
		}
...
function uploadSuccess(file, serverData) {
	try {
		...
		if (serverData.substring(0, 7) === "FILEID:") {
			if(ii[fid]==2)
			{
			addImage("upload_photo_preview.php?id=" + serverData.substring(7));
			}
...
function uploadComplete(file) {
	try {
		...
		rw=this.customSettings.thumbnail_width;
		rwidth=rw.split("|");
		rh=this.customSettings.thumbnail_height;
		rheight=rw.split("|");
		
		if(ii[fid]!=rwidth.length)
		{
	this.requeueUpload(fid);
	
		 this.startResizedUpload(fid,rwidth[ii[fid]],rheight[ii[fid]], SWFUpload.RESIZE_ENCODING.JPEG, this.customSettings.thumbnail_quality, false);
	}
	...
	ii[fid]=ii[fid]+1;
		if (this.getStats().files_queued > 0) {
			...
			progress.setStatus("The image has been uploaded successfully.");
			progress.toggleCancel(false);
