...
$flag_free=false;
	...
	if(!$dr->eof)
	{
		...
		if(($site_subscription==true and isset($_SESSION["people_login"]) and user_subscription($_SESSION["people_login"],(int)$_GET["id_parent"]) and bandwidth_user($_SESSION["people_login"],0)+$subscription_plus<=bandwidth_user($_SESSION["people_login"],1)) or $flag_free)
		{
            ...
            bandwidth_download_add($_SESSION["people_login"]);
            if(!$flag_free)
			{
			...
			if(!$rs->eof)
			{
				...
				if($width!=0)
				{
					$file_redirect=site_root.server_url((int)$_GET["server"])."/".(int)$_GET["id_parent"]."/photo_".$rs->row["size"].".jpg";
					
					if(!file_exists($_SERVER["DOCUMENT_ROOT"].$file_redirect))
					{
							
						if(!$flag_storage)
						{
							$size = getimagesize($_SERVER["DOCUMENT_ROOT"].site_root.server_url((int)$_GET["server"])."/".(int)$_GET["id_parent"]."/".$dr->row["url"]);
							...
							@easyResize($_SERVER["DOCUMENT_ROOT"].site_root.server_url((int)$_GET["server"])."/".(int)$_GET["id_parent"]."/".$dr->row["url"],$_SERVER["DOCUMENT_ROOT"].$file_redirect,100,$width);
						}
						else
						{
							...
							@easyResize($remote_file,$_SERVER["DOCUMENT_ROOT"].$file_redirect,100,$width);
