...
{
$sql="select id_parent,title,description,keywords,author,data,viewed,userid,folder,holder,source,format,duration,content_type,free,downloaded,rating,model,server1,category2,category3,google_x,google_y,url from audio where published=1 and id_parent=".(int)$id_parent;
$rs->open($sql);
if(!$rs->eof)
{
...
//Preview URL
$photo=show_preview($rs->row["id_parent"],"audio",3,1,$rs->row["server1"],$rs->row["folder"]);
$audio=show_preview($rs->row["id_parent"],"audio",2,1,$rs->row["server1"],$rs->row["folder"]);



$preview=show_preview($rs->row["id_parent"],"audio",2,0,$rs->row["server1"],$rs->row["folder"]);
...
$boxcontent=str_replace("{PHOTO}",$photo,$boxcontent);
	...
	if($site_downloadsample==true and !preg_match("/icon_audio/",$audio))
	{
