...
$svector=false;
$lvideo=videolimit;
$lpreview=previewvideolimit;
$laudio=audiolimit;
$lpreviewaudio=previewaudiolimit;
$lphoto=photolimit;
$lvector=vectorlimit;
...
<div class="profile_separator"></div>
	<ul>
		<li  id="icons_upload" <?if($site=="upload"){?>class="activno"<?}?>><a href="upload.php"><span><?=word_lang("upload files")?></span></a></li>
			<ul style="<?if($site=="upload" or $site=="upload_java" or $site=="upload_flash" or $site=="upload_jquery" or $site=="upload_video" or $site=="upload_audio" or $site=="upload_vector" or $site=="upload_category"){echo("display:block");}else{echo("display:none");}?>">
				...
				<?if($scategory==true){?>
					<?if($site_examination and $_SESSION["people_exam"]!=1){}else{?>
...
$rs->open($sql);
if($rs->rc>0){$new_message="<div><b>".$rs->rc."</b></div>";}
