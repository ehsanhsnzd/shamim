...
if($id_parent!=5)
{
    ...
    if(!$dr->eof)
    {
 	...
 	$count_c=0;
 	$search_content=
 '<p class="mt-10"><a href="category/'.$id_parent.'.html" class="btn btn-animated btn-lg btn-gray-transparent home_var_title"> '.$search_title.' </a></p>
 <section class="variable slider">';

 $sql.=" order by id_parent DESC limit 0,18 ";
	$rs->open($sql);
...
//Smarty
    ...
    if (!$rs->eof) {
        ...
        while (!$rs->eof) {
           ...
           $ph[]=array( $rs->row["id"],$rs->row["server1"], $rs->row["folder"],$rs->row["module_table"]);
