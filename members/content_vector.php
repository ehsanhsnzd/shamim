...
{
$sql="select id_parent,title,data,published,description,folder,featured,keywords,author,viewed,userid,free,downloaded,rating,model,flash_width,flash_height,flash_version,script_version,server1,category2,category3,google_x,google_y,url from vector where published=1 and id_parent=".(int)$id_parent;
$rs->open($sql);
if(!$rs->eof)
{
...
//Previews swf
$vector=show_preview($rs->row["id_parent"],"vector",3,0,$rs->row["server1"],$rs->row["folder"]);
$vector_preview=show_preview($rs->row["id_parent"],"vector",3,1,$rs->row["server1"],$rs->row["folder"]);

//Previews jpg
if(preg_match("/icon_vector/",$vector_preview))
{
	$vector=show_preview($rs->row["id_parent"],"vector",2,0,$rs->row["server1"],$rs->row["folder"]);
	$vector_preview=show_preview($rs->row["id_parent"],"vector",2,1,$rs->row["server1"],$rs->row["folder"]);
	if($site_zoomer and !preg_match("/icon_vector/",$vector_preview))
	{
		$vector_preview_url=show_preview($rs->row["id_parent"],"vector",2,1,$rs->row["server1"],$rs->row["folder"],false);
...
//$boxcontent=str_replace("{DOWNLOADSAMPLE}",site_root."/members/sample.php?id=".$rs->row["id_parent"],$boxcontent);
	...
	if($site_downloadsample==true and !preg_match("/icon_vector/",$vector_preview))
	{
