...
if (isset($invoice)) {
    ...
    $fetch_invoice = mysqli_query($connection, "SELECT * FROM subscription_list WHERE id_parent = $invoice");

    $fetch_amount = mysqli_fetch_assoc($fetch_invoice);
    $amount = (int)$fetch_amount['subtotal'];
    $data = $fetch_amount['user'];
...
if (isset($_POST['PayRequestButton']) || !empty($invoice) )
{
    ...
    $localTime = $time;
    $additionalData = $data;
    $callBackUrl =  'http://shamimgraphic.ir/members/callback.php';
    $payerId = 0;
